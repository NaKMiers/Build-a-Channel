...
- Scene E: a **Google Sheets** spreadsheet spotlit on a theater stage = "the star of the show"

## Scene Map (v2: digital UI floated as a screen on REAL desk photos)
| Scene | Track | Time | Base | Beat |
|---|--:|---|---|---|
| A | 1 | 0–10.54 | real white desk photo + CSS Google Meet grid (floating screen) | visible work = rewarded; reply / meetings / "just circling back" |
| B | 3 | 10.5–19.14 | real photo (empty room wall) | thinking looks like nothing; staring at a wall, possibly blinking |
| C | 4 | 19.1–25.8 | real marble desk photo + CSS poll card (floating screen) | most managers can't tell who's productive → reward what they see |
| D | 5 | 25.76–35.08 | real dark-wood desk photo + CSS Trello board (floating screen) | perform motion; move a task column→column; updates about future updates |
| E | 6 | 35.04–42.859 | real red theater-curtain photo + spotlight + Sheets | productivity theater; the star is a spreadsheet |
...
## Motion rules
- Ordinary labels hard-show on the spoken word; `smash()` reserved for punchlines
  (circling back, nothing, blinking, reward stamp, perform motion, theater, the star).
- Poll bars grow on cue (15% / 85%) for a real-survey feel.
- Trello card translates DOING→DONE over 1.5s on "move a task from one column to another column" (30.56).
- Star badge pops on the spreadsheet on "the star" (40.76).
- WIT ~1/2 frame, off-bottom, faces in safe area; 5 distinct poses across the section.

## Channel rules
- No-face: all people rendered as initials avatars / WIT only; the wall photo is people-free.
- Real-UI: editorial depiction of Meet / Trello / Sheets via real icon PNGs (owner-approved technique).
- Subtitle-safe lower margin kept clear of persistent labels.
