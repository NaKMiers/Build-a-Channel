...
Status: `master visual plan in progress` (Section 1 planned; Sections 2-9 not planned)
Source skill: `visual-plan`
Source files: `02-script.md` (rev 2), `03-voiceover.md`
...
|---:|---|---|---:|---:|---:|---|
| 1 | Hook: The Trophy Prints A Receipt | `planned` | 35.904s | 7 | 12 | `visual-plan/section-01-hook/section-01-hook-visual-plan.md` |
| 2 | Reframe: A Purchase, Not An Investment | `not planned` | 33.728s | - | - | - |
| 3 | The Promise Machine | `not planned` | 60.779s | - | - | - |
| 4 | FIFA Keeps The Money | `not planned` | 62.101s | - | - | - |
| 5 | The Three Drains | `not planned` | 55.851s | - | - | - |
| 6 | The Morning After | `not planned` | 61.440s | - | - | - |
| 7 | Who Decides Is Not Who Pays | `not planned` | 66.987s | - | - | - |
| 8 | Payoff: Check The Receipt | `not planned` | 39.573s | - | - | - |
| 9 | Outro: The Cheapest Host On Earth | `not planned` | 17.877s | - | - | - |
...
Not planned yet. Run `visual-plan` per section (or All) to add them here.

## Cross-Section Continuity

- Reused assets (filename -> scenes): `trophy-gold-parody.png` -> S1 (1.1/1.2/1.3/1.5/1.6),
  planned to return in S2 (price-tag swap), S7 (podium), S8 (mantel); `receipt-endless-roll.png`
  -> S1 (1.3/1.6/1.7), planned to carry item lines in S3/S4/S6 and the final close-up in S8;
  `wit-fan-flag-cheer.png` / `wit-fan-frozen-mid-cheer.png` -> S1 fan kit (thumbnail candidates).
- Recurring motif / callback scenes: receipt born S1.3 -> items print at each section's gag
  beat -> S8 full itemized close-up (`WHO KEEPS THE TICKETS?`); the S9 outro prints `TOTAL: $0.00`.
- Mascot emotional arc across the video: euphoric fan (S1) -> squint -> frozen -> hypnotized
  (S3) -> mayor signing (S4) -> horror at drains (S5) -> deadpan party hat (S6) -> tiny
  taxpayer (S7) -> calm resigned nod (S8) -> light CTA host (S9).

## Stale / Regeneration Notes

- Creating this Section 1 plan makes Section 1's implemented assets and render stale - none
  exist yet (no `assets/`, no `previews/`), so nothing to regenerate.
- Section 1 timing is pinned to `voiceover/section-01-hook/section-01-word-timings.json`
  (whisper-tiny.en, 2026-07-02). If Section 1 audio regenerates, re-run timings and re-pin.

## Next Step Boundary

Next workflow step: `visual-implement` (creates the assets named here), then `render`.

Do not continue into visual-implement or render until the user asks for the next skill.
