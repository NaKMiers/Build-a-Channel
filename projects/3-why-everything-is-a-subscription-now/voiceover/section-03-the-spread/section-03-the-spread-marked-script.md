...
- `[deadpan]` on the closing gag "Your free trial of a warm bottom has expired."

## Rhythm style applied in the TTS input (user's anti-monotone toolkit)

- Trailing `...` on setup phrases ("Now it just...", "the second you stop paying...", "Together?")
  so the voice lifts and hangs, then the payoff lands on its own line.
- Stacked `...` lines for the big dramatic holds: before "holds itself hostage", before "POV: you
  own nothing", and the deadpan "warm bottom" gag.
- `. .` inline staccato reset on punchy short lines ("Same program. .", "It's five. .",
  "Each one is small. .") so they land separately instead of blurring.
- Fast flowing comma-run on the subscription list ("One for shows, one for movies...") for the
  "it piles up" rush, then a hard stop.
- Measured behavior: ellipses add real time; `. .` resets prosody without adding length; commas
  and line breaks do not add pauses; `_` does nothing.
