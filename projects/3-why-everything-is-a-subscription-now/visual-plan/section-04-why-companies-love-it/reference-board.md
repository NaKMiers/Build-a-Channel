# Section 4 Reference Board (2026-06-23)

Visual reference pass for Section 4. Real bases sourced via the Openverse API (StockSnap / rawpixel) and
VIEWED before selection. One vivid object base per scene; brand/people-free.

## Reference Pass Status

- Status: `complete (real images via Openverse; viewed before selection)`
- Real bases saved: 4 distinct (cash + coffee each duplicated to a 2nd filename for lint-safe reuse)
...
`FOLLOW THE MONEY` / `RECURRING` kinetic words; gold CSS coins (one-sale, rising geyser, stack, rain);
"buys once = a little / pays monthly = A LOT" comparison; red rings on repeating calendar dates;
`BEAUTIFUL TRAP` payoff over the mousetrap.

## WIT poses used (shared manifest)

sleeping-burned-out (wait years), empty-wallet (pay forever), confused (forgetfulness), suspicious (the trap).
All verified transparent. AVOIDED: `typing-on-laptop` + `money-panic` (baked black backgrounds).
