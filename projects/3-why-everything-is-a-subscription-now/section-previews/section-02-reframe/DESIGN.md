...
Rebuilt completely on the approved Section 1 standing template after the owner asked to base S2 on S1.
The prior build broke the template the same way S1 v1 was rejected: ONE phone base graded 4×, repeated
cream label boxes carrying every idea, and small/low WIT. This build uses `vivid on-topic OBJECT bases
-> VARIED CSS idea-devices -> giant WIT that VARIES per scene`.
...
apps-phone (defuse) → vinyl crate (own) → phone+paywall (rent) → padlock (lock) → device flat-lay
(question). All CC0, brand/people-free, viewed before selection.

## Voice / timing

- Pinned to `voiceover/section-02-reframe/section-02-word-timings.json` (whisper-tiny.en); duration 37.909 (the JSON tail overshoot to 39.16 is ignored).
- Key pins: OWN @10.78 ("paid once"); RENT @17.24 ("rent"); betrayed smash + LOCKED @23.44 ("a little padlock appears"); RENT tags @31.04/31.58/32.24/33.26; payoff @36.26.

## Motion / WIT

- hard-show labels; impact for the OWN/RENT stamps, the betrayed WIT entrance, the payoff; the struck banner draws via a scaleX tween.
- 4 WIT beats, ≤1/scene, BS3 breathes. VARIED side/pose, all GIANT (≈1/2+ frame, high anchor): facepalm RIGHT (1300) → thinking LEFT (1300) → betrayed CENTER (1200) → suspicious RIGHT (1300). Faces/glasses clear of labels, only legs cropped.
