...
dollar spread → phone → coin jar → $1 close-up → phone bank-app. The closer recaps the money/device motif;
the two money shots are distinct images and the phone returns only non-adjacently for the script's literal
final image (the salary screen). 2nd filename keeps lint clean.

## Voice / timing

- Word timings GENERATED this run (whisper-tiny.en); pinned to word starts; capped at 54.101. Key pins: YOUR FORGETTING @9.8, PRODUCT:YOU @16.5, a DESIGN @24.5, statement @31.5, CANCEL THE GHOSTS @38.5, bank-app @47.5, "your salary. for now." @52.6.

## Motion / WIT

- hard-show labels; impact for YOUR FORGETTING, the product tag, a DESIGN, CANCEL THE GHOSTS, the payoff.
- 4 giant WIT beats, varied side/pose (thinking R → shocked L → holding-receipt R → deadpan-side-eye R); S3 breathes. AVOID `typing-on-laptop` / `money-panic` (baked black bg).
- Final-frame collision verified @53.2s (payoff centered, clear of right WIT + left bank-app card).
