# Section 6 Visual Plan (Section 1 standing template, 2026-06-23)

Video: `Why Everything Is a Subscription Now`
Section: `Section 6: Easy In, No Way Out`
Status: `built + previewing on localhost:1006`

## Source Inputs

- Voiceover: `voiceover/section-06-easy-in-no-way-out/scratch-audio/section-06-...-david23-am_eric-0.8.mp3`
- Duration: `53.013s` (TTS). Word timings GENERATED this run (whisper-tiny.en); clean; tail capped at 53.013.

## Narration

```text
Okay, you say. I'll just cancel. Oh, sweet child.
...
|---|---:|---|---|---|---|
| BS1 easy-in/hard-out | 0.0–9.0 | "I'll just cancel… 10s vs a vision quest" | stopwatch | the time asymmetry | base-stopwatch.jpg |
| BS2 the maze | 9.0–25.7 | "glowing button vs final boss… account→…→a phone number" | wooden labyrinth | cancelling is a maze you must beat | base-maze.jpg |
| BS3 negative option | 25.7–33.7 | "it has a name: negative option billing… unless you say stop" | contract / fine print | the named trick | base-contract.jpg |
| BS4 burden / −1000 aura | 33.7–42.3 | "pushed onto you… −1000 aura… didn't do anything wrong" | maze, dark (callback) | still lost, penalised for nothing | base-maze-2.jpg |
| BS5 give up / no salary | 42.3–53.013 | "give up… a part-time job. with no salary… also not a subscription" | stopwatch, dark (callback) | the time cost = an unpaid job | base-stopwatch-2.jpg |
...
| C7 | 33.9 / 37.4 / 39.1 | "pushed onto you / −1000 aura / didn't do anything wrong" | BS4 | headline; game-HUD `−1000 AURA`; aside | hard-show / impact | confused L @35.2 |
| C8 | 42.3 / 47.4 / 49.1 / 50.4 | "give up / a part-time job / no salary / not a subscription" | BS5 | headline; label; red `salary $0`; payoff | hard-show / impact | tiny-defeated R @43.1 |

## WIT Pose Plan (giant, varied)

| Time | Pose | Side / scale | Why |
|---:|---|---|---|
| 13.0–25.7 | wit-pose-running-away.png | RIGHT, width 1020, bottom:-280 | fleeing the menu maze |
| 28.0–33.7 | wit-pose-suspicious.png | RIGHT, width 1080, bottom:-300 | the sneaky named trick |
| 35.2–42.3 | wit-pose-confused.png | LEFT, width 1180, bottom:-330 | lost, penalised (−1000 aura) for nothing |
| 43.1–53.0 | wit-pose-tiny-defeated.png | RIGHT, width 1080, bottom:-320 | gave up |

WIT density: 4 beats / 5 scenes; BS1 breathes. All verified transparent. AVOID `typing-on-laptop` + `money-panic` (baked black bg).
...
Rejected: a phone shot with a person's head/face (no-face); a Holocaust-memorial photo mis-tagged "maze"; a cluttered museum punch-clock display; PNG sticker illustrations; war "white flag" photos. See `reference-board.md`.

## HyperFrames Guidance

- Composition `Section06Easy`, 1920x1080, 53.013s, port 1006.
- Scenes on tracks 1/3/4/5/6 (cross-fades at 9.0/25.7/33.7/42.3); cues on track 2 (cue-d trimmed to 8.58 to avoid the 42.3 float overlap).
- Callback bases use 2nd filenames + a dark grade so they read as new beats, not repeats.
- Menu chips kept in the left 2/3; WIT running-away on the right (verified no overlap at 23.0s).
- No emoji glyphs except the ☎ phone (renders as text; verified). Smashed elements use explicit left/top.
- Snapshot QA: 4 / 8 / 13 / 19 / 23 / 28 / 32 / 38 / 46 / 51.5.

## Review-Prevention Checklist

- voice sync pinned to generated word starts: yes
- vivid object bases (stopwatch, maze, contract); the 2 callbacks are deliberate, darker, distinct (documented + swappable): yes
- varied idea-devices, not repeated cream boxes: yes
- giant WIT, varied side/pose (R/R/L/R), 4 beats, S1 breathes: yes
- menu chips don't collide with WIT (left 2/3 vs right): yes
- markup means something (the menu trail = the actual cancel steps; the stamp names the trick): yes
- impact reserved for the vision-quest label, FINAL BOSS, the stamp, −1000 aura, the payoff: yes
