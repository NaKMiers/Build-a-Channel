...
## Structure (6 big scenes, ~13 cues)
...
gift box → keyboard desk → hourglass → everyday desk → wallet → piggy. All CC0, all distinct (review fix:
S1/S4 were a reused blank phone). The hourglass literally IS "the countdown."

## Voice / timing

- Word timings GENERATED this run (whisper-tiny.en); pinned to word starts; duration capped at 53.867. Key pins: FREE splash @2.9, card @6.3, countdown @16.4, $2.99 flip @19.1, reminder fades @23.5, DESIGN @26.0, ghost charges @34.0–35.2, statement @40.2, ring @43.3, EXPIRED @50.2.

## Motion / WIT

- hard-show labels; impact for the price flip, the red ring, the labels, and the EXPIRED banner.
- 4 giant WIT beats, varied side/pose (deadpan-side-eye R → hidden-fee-panic L → thinking R → holding-receipt-evidence L); S1 + S5 breathe. AVOID `typing-on-laptop` / `money-panic` (baked black bg).
- The one red ring is meaningful: it targets the exact `?? UNKNOWN −$3.00` statement row (snapshot-verified), not empty space.
