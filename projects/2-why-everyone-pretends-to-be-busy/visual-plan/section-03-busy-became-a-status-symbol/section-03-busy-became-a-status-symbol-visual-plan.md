# Section 3 Visual Plan (remade from scratch)
...
## Source Inputs
- Voiceover: `voiceover/section-03-.../...david23-am_eric-0.84.mp3` (45.14s, 0.84)
- Word timings: `voiceover/section-03-.../section-03-word-timings.json` (existing; clean)
- NOTE: audio 0.84 (plain) vs Sections 4–7 at 0.86 (pause-tuned). Mismatch flagged; re-pin if regenerated.

## Visual Direction (new standard)
- 6 scenes, each on a REAL people-free photo base; real-UI for "I'm so busy" (chat), "no free time" + "meetings about meetings" (calendar).
- WIT GIANT (~1/2 frame) and HIGH (head+torso inside frame, legs cropped); labels arranged around it.
- WIT path (6 distinct): awkward-celebration → talking → deadpan → suspicious → thinking → tiny-defeated.
...
|---|---:|---|---|---|---|---|
| C1 | 0–2.48 | "status symbol"@2.48 | A | BUSY = STATUS SYMBOL | impact | awkward-celebration @0.4 |
| C2 | 4.06 | "I'm so busy"@4.06 / important@7.84 | B | chat "I'm so busy!!" → (= please notice I'm important) | hard-show | talking-front @4.3 |
| C3 | 11.0 | "busy"@11.98 / emotion@13.32 / tired@15.28 | C | EMOTIONS board + BUSY? ; NOT AN EMOTION ; (everyone's tired) | impact | deadpan-side-eye @11.2 |
| C4 | 17.0 | holidays@19.34 / no free time@23.06 / brag@25.42 | D | LONG HOLIDAYS / FREE AFTERNOONS → packed-calendar NO FREE TIME → OPPOSITE BRAG | impact | suspicious @17.2 |
| C5 | 25.42 | responsible@26.40 / needed@27.68 / waiting@29.80 / valuable@31.84 | E | RESPONSIBLE → NEEDED → WAITED-FOR → = FEELS VALUABLE (staggered) | hard-show | thinking @25.6 |
| C6 | 33.32 | day@33.32 / meetings@36.96 / near the work@43.75 | F | calendar fills; MEETINGS ABOUT MEETINGS → PROVING YOU'RE NEAR THE WORK | impact | tiny-defeated @33.6 |
...
- 1920x1080; 6 scenes (tracks 1/3/4/5/6/7), 6 cue groups (track 2); audio 45.14s.
- Cues pinned to word-timings; the "busy sounds" labels (E) and the calendar events (F) stagger on their words.
- Real-UI chat/calendar = CSS over real photos; the CSS "emotions board" (C) lists real emotions with BUSY? added in red.
- WIT GIANT + HIGH (side width ~1300–1340, bottom≈-350); labels to the opposite side/top so WIT covers nothing.
- Must not invent: bases, label text, word-timed order, WIT poses.
- QA snapshots: 2.6 / 6 / 8.5 / 14 / 20 / 24 / 27.5 / 31 / 37 / 44s.

## Approval Checks
- "Busy is not an emotion" lands as a dry board; "opposite brag" stays light; final lands "near the work."
- WIT giant + high in every scene; labels arranged around it. Real bg every scene; brand/people-safe.
