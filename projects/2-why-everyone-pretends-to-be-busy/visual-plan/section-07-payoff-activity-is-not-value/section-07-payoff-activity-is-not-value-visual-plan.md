...
Section: `Section 7: Payoff: Activity Is Not Value`
Status: `draft visual plan for approval`

## Section Goal
Land the insight calmly and tie the motif together: modern life confuses activity with value; the fix is not "be less busy" but "be more honest about what matters." End on the calendar-with-Wi-Fi motif.

## Source Inputs
- Voiceover: `voiceover/section-07-.../...david23-am_eric-0.86.mp3` (46.78s, 0.86)
- Word timings: `voiceover/section-07-.../section-07-word-timings.json` (generated; tail fixed; cue times REAL)

## Visual Direction (real backgrounds + real-UI, calmer payoff)
- 5 scenes; each on a REAL people-free photo (hands OK) or a real-UI screen that IS the descriptive image.
- Ties motifs: ends on a real-UI **Google Calendar** packed with red "URGENT" events = "a calendar with Wi-Fi."
- WIT path (5 distinct, calmer): thinking → talking-front → typing-on-laptop → deadpan-side-eye → trapped-by-app-screen.
...
|---|---|---|---|
| base-question.jpg | StockSnap CC0 ("?" note + crumpled paper) | safe base | A (+ E behind calendar) |
| base-busy.jpg | StockSnap CC0 (hands typing, wood) | safe base | B (+ D behind chat) |
| base-culture.jpg | Wikimedia CC BY 2.0 (laptop in warm office) | safe base | C |
| icons/gcal.png | Wikimedia (Google Calendar logo) | real-UI icon | E calendar header (editorial) |
| 5 WIT poses | shared manifest | safe asset | one per scene |

Rejected: ss-OGBUJERCEV / ss-AUNO6KK0TL (Apple logos), ss-MY9TVNEESX (marble too like S5), people-photos (no-face).

## HyperFrames Guidance
- 1920x1080; 5 scenes (tracks 1/3/4/5/6), 5 cue groups (track 2); audio 46.78s.
- Cue starts pinned to (tail-fixed) word-timings; list rows (C) and label trios (B) reveal item-by-item on their words.
- Boards/labels/chat/calendar = CSS over the real photo; light scrim only, no gray-wash.
- Payoff is calmer: mostly hard-show; impact reserved for `≠`, SAFER, BE HONEST, and the final A CALENDAR WITH WI-FI.
- WIT ~1/3–1/2 frame, faces safe; the closing `trapped-by-app-screen` WIT carries the final emotion. gcal icon editorial.
- Must not invent: bases, label text, word-timed order, WIT poses.
- QA snapshots: 4.5 / 7.5 / 11 / 15.5 / 20.5 / 25 / 31.5 / 37 / 40 / 45.5s.

## Approval Checks
- `ACTIVITY ≠ VALUE` clearly visible; payoff calmer than earlier chaos.
- Ends on the calendar-with-Wi-Fi motif with WIT trapped.
- Real backgrounds + real-UI; brand/people-safe; word-timed.
