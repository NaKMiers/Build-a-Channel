...
- Use **visual-plan** when the user asks for visual plan, scene plan, scene-by-scene screen direction, describe the visuals, or step 4 of the main video workflow. It builds ONE master plan + synced per-section copies, breaks the script into per-sentence (or few-sentence) scenes, describes each scene in extreme detail (composition, elements, mascot pose, on-screen text, emotion, insight, linkage, show-as-you-say timing, sound, color), and lists each scene's ASSETS (type generate/browse/screenshot/reuse, filename, layout). It DESCRIBES only - it never writes image-generation prompts - and may invent new poses/scenes (within copyright, law, and YouTube community standards).
- Use **visual-implement** (step 4.5, after visual-plan, before render) when the user asks to implement the visual plan, create the assets, generate the scene images, build the assets folder, or prep assets for render. It reads each scene's ASSET list and produces them: writes the image-generation prompt and creates an isolated element for `generate` assets, browses a license-safe real photo / captures a public screenshot for `browse` assets, copies poses, and REUSES any asset already made (by filename - keeps recurring characters identical). Saves isolated elements (never a composed scene) into `assets/` + `assets/asset-manifest.md`.
- Use **render** when the user asks for render, HyperFrames build, create video from visual plan, section preview, localhost preview, section MP4 render, or step 5 of the main video workflow. It COMPOSITES the mascot + the pre-made assets from `assets/` (created by visual-implement) into each scene's layout; it re-sources an asset only as a documented fallback when one is missing.
