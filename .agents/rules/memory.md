# Memory Rules

The project should improve after every serious review, production pass, and upload.

## Memory Levels

- `projects/<slug>/`
  Current-video decisions, timestamps, review notes, assets, voiceover, renders, and upload details.
- `.agents/_shared/channel/learning-log.md`
  Reusable lessons, operational discoveries, experiments, and dated decisions.
- `.agents/_shared/`
  Reusable systems, templates, tools, assets, and workflow docs.
- `.agents/skills/<skill>/`
  Future skill-specific memory once sequential production skills exist.

## Promotion Rule

Store a lesson at the narrowest useful level first.

Promote it upward only if it can improve future videos.

## Classification

Before writing strategy into shared memory, classify it as:

- `Core`
- `Experiment`
- `Reject`

Only `Core` belongs in the channel foundation.

Use `Experiment` for uncertain ideas in the learning log.

Do not persist `Reject` ideas.

## Self-Improvement

After a skill or workflow run, future review feedback should update:

1. the active project file,
2. the relevant shared system or workflow,
3. the skill's own memory, if the feedback is specific to that skill.

The goal is not just to remember mistakes.

The goal is to make the next run better.
