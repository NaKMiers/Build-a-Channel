...
Turn a selected `00-topic-intake.md` and `01-research-pack.md` into a sectioned working script that is ready for packaging, then voiceover.
...
If any exist, list them as stale in chat and tell the user they should be removed or regenerated by rerunning downstream skills in order, starting with `packaging`.
...
7. Stop before packaging, voiceover, visual plan, HyperFrames, renders, upload, or self-learning.
...
Next workflow step: `Packaging`

Do not continue into packaging, voiceover, visual plan, HyperFrames, renders, upload, or self-learning until the user asks for the next skill or explicitly requests that step.
