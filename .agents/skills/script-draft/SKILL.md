...
When this skill creates, updates, or reruns `projects/<slug>/02-script.md`, every later output in the same project becomes stale — including `output/packaging.md`, because packaging now depends on the script. List stale downstream files in chat. Do not silently delete them. Remove stale downstream files only when the user explicitly asks; otherwise downstream skills must be rerun in order.
...
After creating, updating, or rerunning `02-script.md`, check the same project for downstream files (new-project numbering; legacy projects use the old numbers — resolve by suffix per `.agents/rules/video-workflow.md`):
...
- `07-upload.md`
- `08-self-learning.md`
- `output/packaging.md` (+ `output/thumbnails/`) — packaging now depends on `02-script.md`, so a script change makes packaging stale too
