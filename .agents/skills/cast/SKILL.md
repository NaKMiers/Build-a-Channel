...
for t in $(grep -oE '^\| @[A-Z]+' "$F" | tr -d '| @'); do
  printf '%-10s %s\n' "@$t" "$(awk -v n="**$t.jpeg**" 'index($0,n){f=1} f' "$F" \
    ...
    | grep -oiE '(hoodie|shirt|parka|tunic|robe|vest|wrap|coat|dress|jacket|singlet|jumper|top|bubble body|casing ring|shell|envelope body|bill body|card body|lever body)[^,]*\(#[0-9A-F]{6}\)' \
    | head -1)"
done
