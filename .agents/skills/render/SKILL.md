...
- BUILD TO THIS BAR ON THE FIRST PASS — do not render a plain version and wait for rejection (owner-confirmed 2026-06-24). On `why-buy-1-get-1-beats-50-off` the owner rejected plain first passes three times before each was rebuilt to this template. Hard requirements every section:
  - dark dramatic base grade: `filter: saturate(~1.1) contrast(~1.08) brightness(~0.42–0.55)` + a heavy radial scrim. Never ship a bare/flat base (plain wood, objects-on-white, a dim antique) — it reads as "backgrounds too simple."
  ...
  - second reusable kit to copy: `projects/why-buy-1-get-1-beats-50-off/section-previews/section-03-the-receipt-knows/index.html` (bignum `$5→$10`, stamp, toast, banner, glowing `payoff` FREE, giant WIT across 5 scenes).
