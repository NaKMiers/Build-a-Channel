...
- Each active video keeps its HyperFrames source in `projects/<slug>/hyperframes/`.
...
- Final and review MP4s still belong in `projects/<slug>/renders/`.
...
- Legacy Remotion notes live in `.agents/_shared/remotion/`; keep them as reference only unless the user asks otherwise.

## Current Active Example

```text
projects/why-everyone-pretends-to-be-busy/hyperframes/
```

This is the current active HyperFrames project folder after the source refactor.
