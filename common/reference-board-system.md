...
4. Create a local board folder from [common/reference-boards/_template](reference-boards/_template).
...
Do not store one-off topic boards in `common/`.
Future one-off boards belong in the relevant `video-projects/<slug>/` folder only after an explicit apply command.
