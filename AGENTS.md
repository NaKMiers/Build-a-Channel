...
- Use [Voiceover](C:\ME\THINGS\Build a Channel\.agents\skills\voiceover\SKILL.md) when the user asks for section voiceover, generate audio for a script section, create narration audio, all section voiceovers, or step 3 of the main video workflow.
- Use [Visual Plan](C:\ME\THINGS\Build a Channel\.agents\skills\visual-plan\SKILL.md) when the user asks for visual plan, scene plan, scene-by-scene screen direction, describe the visuals, or step 4 of the main video workflow. It builds ONE master plan + synced per-section copies, breaks the script into per-sentence (or few-sentence) scenes, describes each scene in extreme detail, and lists each scene's ASSETS (type generate/browse/screenshot/reuse, filename, layout). It DESCRIBES only - it never writes image-generation prompts - and may invent new poses/scenes (within copyright, law, and YouTube community standards).
- Use [Visual Implement](C:\ME\THINGS\Build a Channel\.agents\skills\visual-implement\SKILL.md) (step 4.5, after visual-plan, before render) when the user asks to implement the visual plan, create the assets, generate the scene images, build the assets folder, or prep assets for render. It reads each scene's ASSET list and produces them: writes the prompt and creates an isolated element for `generate` assets, browses a license-safe real photo / captures a public screenshot for `browse` assets, copies poses, and REUSES any asset already made (by filename, keeping recurring characters identical). Saves isolated elements (never a composed scene) into `assets/` + `assets/asset-manifest.md`.
- Use [Render](C:\ME\THINGS\Build a Channel\.agents\skills\render\SKILL.md) when the user asks for render, HyperFrames build, create video from visual plan, section preview, localhost preview, section MP4 render, or step 5 of the main video workflow. It COMPOSITES the mascot + the pre-made assets from `assets/` (created by visual-implement) into each scene's layout; it re-sources an asset only as a documented fallback when one is missing.
...
- Use [Packaging](C:\ME\THINGS\Build a Channel\.agents\skills\packaging\SKILL.md) (after caption) when the user asks for title, thumbnail, packaging, YouTube description, tags, hashtags, upload metadata, A/B title/thumbnail testing, or thumbnail prompts. Packaging requires `00-topic-intake.md`, `01-research-pack.md`, and `02-script.md`; its recommended position is after `caption` so it can also package shorts and use real chapters. It produces 5 LOCKED title+thumbnail A/B pairs for the main video (title N coupled to thumbnail N), the description/tags, and - when built shorts exist - one title/description/thumbnail per short. Everything goes into one file `output/packaging.md`; images to `output/thumbnails/`. No longer numbered (was `03-packaging.md`); no separate `PROMPTS.md`.
...
- main production skills run in order: `topic-intake -> research-pack -> script-draft -> voiceover -> visual-plan -> visual-implement -> render -> review -> combine -> caption -> packaging -> upload -> learning`
- `visual-implement` is a main-pipeline step but UNNUMBERED (its deliverable is the `assets/` library + `assets/asset-manifest.md`), so it does not renumber any markdown files; it runs after `visual-plan` and is a hard gate before `render`
...
- Competitive advantage: `interesting English` - entertainment-first explainers; learners come for the fun and improve their English as a side effect
- Topic sourcing: `trending / currently-interesting topics with real demand evidence (see topic-intake)`
- Main lane: `money, internet, society, business, modern life, and current culture`
- Tone: `smart, simple, funny, dry, and allowed savage/cheeky` (edge aimed at the system / the viewer's own wallet, never slurs; public figures as transformative parody) - see `.agents/_shared/channel/learning-log.md`
- Default video text style: `handwritten labels and captions rendered through HyperFrames`

## Text Formatting Rule (hard)

NEVER use the em dash (Unicode U+2014) anywhere in this project - not in any file, not in chat replies.
Always use a plain hyphen "-" instead. The owner never wants to see that character.
