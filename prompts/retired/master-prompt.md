...
- **`@YOU` is always TOSS — the channel mascot.** TossExplains has one permanent character, Toss, and he is the viewer stand-in in every single video. He is never re-derived from the script, never redesigned, and never swapped for a different stand-in. His canonical sheet is `MASCOT.jpeg`, and the full identity lock lives in `character-prompt.md`.
